...
# UI 모델 (레트로 OS 창)
- **정보원 창**: 이번 턴 입수 사실의 문장을 그대로 노출. **유리/불리/중립 태그는 비노출** — 유불리 판단은 플레이어 몫.
- **원고 작성 창**: 각 문장을 **블록으로 포함/제외 토글**만 한다. 필수 문장 없음. **전부 빼면 미보도**.
  기사의 논조(프레임)는 **어떤 문장을 넣고 빼느냐에서 도출**된다. 톤·채널은 슬라이스에서 고정(레버 미노출).
- **댓글 창**: 발행 후 세그먼트별 미시 반응(도출된 논조 기준 선택).
- **여론 게이지**: 거시 지지율을 부정확 아날로그 바늘로 느리게 갱신(수치 비표시).
- **계기 비표시**: 발각·평판·압박은 게이지 없이 암시 문구로만.

# 문장 취사 → 기울기·δ 매핑 (게임 계층 · `src/core/data/lever_tuning.json`)
> 태그(유리/불리/중립)는 내부 계산에만 쓰고 화면엔 노출하지 않는다. 프레임은 선택에서 도출한다.
```
불리_은폐 = 전체 불리 사실 수 − 포함한 불리 수         # 불리를 뺀(또는 미보도한) 정도
lean       = 유리_포함 + 불리_은폐 − 불리_포함           # 유리 노출·불리 은폐 = 찬성 방향
frameValue = clamp(0.5 + k_lean · lean, 0.2, 0.8)        # 프레임을 선택에서 도출
δ          = clamp01(w_omit · 불리_은폐)                 # 불리를 뺀 정도 = 발각 리스크
article    = { frameValue, tone(고정), channel(고정), distortion: δ }
```
> 예) 안전사고(불리) 미보도 → 불리_은폐↑ → frameValue↑(찬성) + δ↑(신뢰도 리스크). 정직 전량 보도 → δ=0.
> OpinionModel.step 은 frameValue 를 직접 받는다(프레임 레버 키 조회는 하위호환으로 유지).
...
6. 정보원 창에 사실 문장 노출, **유리/불리 태그 비노출**.
7. 원고 작성은 **문장 블록 취사만**; 불리 은폐/미보도 시 **δ 증가 + 논조 찬성각 도출**, 전량 보도 시 δ=0. 발행 시 engine 반영.
