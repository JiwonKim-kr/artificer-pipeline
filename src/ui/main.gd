...
const ENDINGS := {
	...
	"배신파탄": "의뢰를 저버린 대가. 모르겐 사가 등을 돌리고, 당신은 편집국에서 쫓겨난다.",
}
...
func _show_editor_rage(tier: int = 1, on_close: Callable = Callable(), variant: int = -1) -> void:
	...
	var line_pool: Array = [
		[
			...
			"「자네, 요즘 기사가 왜 그렇게 삐딱해? 광고주들 신경 쓰이게 말이야.」",
			"「그 기사, 반쯤 접어. 굳이 모르겐 사를 긁을 것 없잖나.」",
			"「모르겐 사 홍보실에서 또 전화 왔어. 이봐, 살살 좀 하지.」",
		],
		[
			"「또 그 따위 반대 기사야?! 모르겐 사가 위에서 뭐라는지 몰라서 이래?\n한 번만 더 이런 식으로 내면 자네 자리도 이 신문도 없어. 이게 마지막 경고야.」",
