...
## 제출작 — 「태엽 인간 (The Automata Act)」

> 진실만으로도, 여론은 조작된다.

석간지 기자가 정보원이 흘린 **진실한 문장만을 취사·배치**해 8턴 안에 여론을 돌리는 디젤펑크 여론 시뮬레이터. 거짓말은 한 줄도 쓰지 않는다.

| | |
|---|---|
| ▶ **플레이 (웹)** | https://jiwonkim-kr.github.io/artificer-pipeline/ |
| **소스 브랜치** | [`game/gireki-sim`](../../tree/game/gireki-sim) |
...
| `game/gireki-sim` | 파이프라인으로 제작한 「태엽 인간」 ← **제출작** |
...
`verify`가 5항목을 통과해야 반영된다 — Godot headless 임포트 / 스모크 테스트 / 네이밍·디렉토리 규칙 / 매니페스트↔실제 파일 정합성 / lore 정본 모순. push·PR마다 CI에서 재검증한다([`verify.yml`](.github/workflows/verify.yml), Godot 4.6.3).
